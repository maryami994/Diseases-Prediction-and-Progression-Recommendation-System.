...
df = pd.read_csv(r"D:\depi_disease_tracking\data\clinically_validated_balanced_disease_data.csv")
...
class ClassificationModelTrainer:
    ...
    def train_xgboost(self):
        ...
        with mlflow.start_run(run_name="XGBoost_Classifier"):
            ...
            mlflow.log_metric("train_accuracy", accuracy_score(self.y_train, y_train_pred))
            mlflow.log_metric("test_accuracy", accuracy_score(self.y_test, y_pred))
        ...
        if self.is_multi_label:
            ...
            for i, label in enumerate(self.target_columns):
                ...
                print(cm[i])
                    ...
                    self._plot_confusion_matrix(y_test=self.y_test, y_pred=y_pred, target=self.target_columns[0])
        
            def _plot_confusion_matrix(self, y_test, y_pred, target):
                """Helper method to plot and print confusion matrix."""
                ...
                sns.heatmap(
                    ...
                    xticklabels=['No Diabetes', 'Type 1', 'Type 2'] if 'Diabetes Type' in target else np.unique(y_test),
                    yticklabels=['No Diabetes', 'Type 1', 'Type 2'] if 'Diabetes Type' in target else np.unique(y_test)
                )
    ...
    def train_neural_network(self):
        ...
        with mlflow.start_run(run_name="Neural_Network_Classifier"):
            mlflow.log_param("learning_rate", 0.0005)
            mlflow.log_metric("train_accuracy", accuracy_score(self.y_train, y_pred_train))
            mlflow.log_metric("test_accuracy", accuracy_score(y_test_np, y_pred_test))
...
def main():
    ...
    mlflow.set_experiment("Depi_OOP_Project")
